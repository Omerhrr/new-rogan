...
// SECURITY: Fail hard if JWT_SECRET is missing
const JWT_SECRET = process.env.JWT_SECRET;
if (!JWT_SECRET) {
  if (process.env.NODE_ENV === 'production') {
    throw new Error('FATAL: JWT_SECRET environment variable is required in production.');
  }
  console.error('WARNING: JWT_SECRET not set. Using insecure dev-only secret.');
}
...
export async function hashPassword(password: string): Promise<string> {
  // SECURITY: Limit password length to prevent bcrypt DoS (max 72 bytes processed by bcrypt anyway)
  if (password.length > 128) throw new Error('Password too long (max 128 characters)');
...
export async function getUserFromRequest(): Promise<{
  ...
  displayName: string | null; avatar: string | null; bio: string | null; isLive: boolean;
} | null> {
  ...
  const user = await db.user.findUnique({
    ...
    select: { id: true, email: true, username: true, role: true, displayName: true, avatar: true, bio: true, isLive: true },
  });

  // SECURITY: Verify role hasn't been elevated since token was issued
  if (user && user.role !== payload.role && payload.role === 'admin') return null;
  return user;
}
...
export function setAuthCookie(token: string): CookieOptions {
  return {
    name: TOKEN_NAME, value: token, httpOnly: true,
    secure: process.env.NODE_ENV === 'production', sameSite: 'lax',
    path: '/', maxAge: 60 * 60 * 24 * 7,
  };
}

export function clearAuthCookie(): CookieOptions {
  return {
    name: TOKEN_NAME, value: '', httpOnly: true,
    secure: process.env.NODE_ENV === 'production', sameSite: 'lax',
    path: '/', maxAge: 0,
  };
}
